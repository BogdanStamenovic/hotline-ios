...
async def test_the_phone_can_poll_ack_and_answer_through_the_daemon():
    """Outcome B end to end over HTTP: the app registers presence, takes a ring,
    proves it showed it, and answers."""
    from hotline_ios.ring.chain import RingChain
    from hotline_ios.ring.local import LocalTransport

    local = LocalTransport(rtp_host="127.0.0.1")
    # Buried inside a chain AND a confirmation wrapper, so the lookup is
    # exercised the way it will actually be assembled.
    chain = RingChain([ConfirmedRing(local, confirm_within=5.0)])
    service = Service(chain, FakePool(), transcriber=FakeTranscriber(),
                      speaker=FakeSpeaker(), segmenter_factory=OneShotSegmenter)
    server = await run_server(service, 18799)
    try:
        # Presence.
        first = await asyncio.to_thread(post, 18799, "/api/v1/device/poll", {"wait": 0})
        assert first["ring"] is None

        async def app():
            page = await asyncio.to_thread(post, 18799, "/api/v1/device/poll", {"wait": 10}, 20)
            ring = page["ring"]
            assert ring is not None and ring["from"] == "a session"
            await asyncio.to_thread(post, 18799, "/api/v1/device/ack", {"call_id": ring["call_id"]})
            await asyncio.to_thread(
                post, 18799, "/api/v1/device/answer",
                {"call_id": ring["call_id"], "answered": True})

        phone = asyncio.create_task(app())
        body = await asyncio.to_thread(
            post, 18799, "/api/v1/call",
            {"reason": "the build is stuck", "source": "a session", "wait": False}, 30)
        await phone
        assert body["state"] == "ringing"
        assert body["rings_when_closed"] is False   # honest about outcome B
    finally:
        await server.close()


async def test_device_routes_are_409_not_404_on_a_server_without_that_transport():
    # A 404 would look like the app having the wrong URL. A 409 says the server
    # is configured differently, which is the actual situation.
    service = Service(LoopbackTransport(FMT), FakePool())
    server = await run_server(service, 18800)
    try:
        with pytest.raises(urllib.error.HTTPError) as exc:
            await asyncio.to_thread(post, 18800, "/api/v1/device/poll", {"wait": 0})
