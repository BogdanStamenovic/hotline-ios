...
async def test_the_first_working_transport_is_used():
    chain = RingChain([named(LoopbackTransport(), "own-app"),
                       named(LoopbackTransport(), "sip")])
...
async def test_it_falls_through_to_the_next_and_says_so():
    ...
    dead = named(LoopbackTransport(reachable=False), "own-app")
    alive = named(LoopbackTransport(), "sip")
...
async def test_a_decline_stops_the_chain_dead():
    ...
    second = named(LoopbackTransport(), "sip")
    chain = RingChain([named(LoopbackTransport(decline=True), "own-app"), second])
...
async def test_ringing_out_does_not_fall_through_by_default():
    ...
    second = named(LoopbackTransport(), "sip")
    chain = RingChain([named(LoopbackTransport(answer=False), "own-app"), second])
...
async def test_everything_failing_is_one_error_naming_every_attempt():
    chain = RingChain([named(LoopbackTransport(reachable=False), "own-app"),
                       named(LoopbackTransport(reachable=False), "sip")])
...
async def test_a_transport_that_raises_something_unexpected_is_survived():
    ...
    chain = RingChain([Broken(), named(LoopbackTransport(), "sip")])
    await chain.ring(WHO)
    assert chain.used == "sip"
...
async def test_an_unconfirmable_chain_refuses_rather_than_trusting_silence():
    ...
    chain = RingChain([
        ConfirmedRing(named(LoopbackTransport(confirms=False, answer=False), "own-app"),
                      confirm_within=0.1),
        ConfirmedRing(named(LoopbackTransport(), "sip"), confirm_within=1.0),
    ])
    await chain.ring(WHO)
...
async def test_rings_when_closed_is_true_if_any_link_can():
    weak = named(LoopbackTransport(), "mention")
    weak.rings_when_closed = False
    strong = named(LoopbackTransport(), "sip")
